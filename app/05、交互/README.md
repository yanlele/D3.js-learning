...
api | 说明
:- | :- 
**d3.zoom** | 创建一个新的缩放行为，并返回该行为。zoom 既是一个对象又是一个函数，通过情况下通过 selection.call 来应用到元素上。.
**zoom(selection)** | 将缩放交互应用到指定的被选中的元素上.
**zoom.transform(selection, transform)** | 修改指定的选中的元素的变换.
...
transform.rescaleX | 将当前坐标变换应用到指定的 x- 比例尺的 domain.
transform.rescaleY | 将当前坐标变换应用到指定的 y- 比例尺的 domain.
transform.toString | 将坐标转换格式化为 SVG 可直接用的字符串.
d3.zoomIdentity | 当前坐标变换标识.
